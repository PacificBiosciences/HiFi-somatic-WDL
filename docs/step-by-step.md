...
curl -LO 'https://zenodo.org/record/17043964/files/hifisomatic_resources.tar.gz'
# Extract the archive
...
├── refFlat.hg38.txt
├── severus_all_samples.octopusv.filled.sorted.AN10.AC4.bcf
├── severus_all_samples.octopusv.filled.sorted.AN10.AC4.bcf.csi
...
The parameters `hifisomatic.report_vis_script` set the path to the script used to generate the final report. The script can be found in the `resources` folder of the Github repository (`visualize_hifisomatic.qmd` and `visualize_hifisomatic_tumor_only.qmd`).
