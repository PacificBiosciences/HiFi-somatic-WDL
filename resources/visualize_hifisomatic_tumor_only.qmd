...
#     # cnv_hap1_file = "../resources/RDES.copynumbers_segments_HP_1.bed",
#     cnv_hap1_file = "../test_data/report_test_file/COLO829_15X.copynumbers_segments_HP_1.bed",
#     # cnv_hap2_file = "../resources/RDES.copynumbers_segments_HP_2.bed",
#     cnv_hap2_file = "../test_data/report_test_file/COLO829_15X.copynumbers_segments_HP_2.bed",
...
cnv1 <- read_tsv(params$cnv_hap1_file, comment="#",
    col_names = c("chromosome", "start", "end", "coverage", "cn", "conf", "Severus_BP")) %>%
    mutate(chromosome = str_replace(chromosome, "chr", ""))
cnv2 <- read_tsv(params$cnv_hap2_file, comment="#",
    col_names = c("chromosome", "start", "end", "coverage", "cn", "conf", "Severus_BP")) %>%
    mutate(chromosome = str_replace(chromosome, "chr", ""))
...
# Helper function to set absolute pos and add label column for plotting
set_absolute_pos_and_label <- function(cnv) {
    cnv <- set_absolute_pos(cnv, cytoband_length = cytoband$cytoband_length)
    cnv <- cnv %>%
        mutate(
            cn_cut = ifelse(cn > 5, 5, cn),
            label = ifelse(cn_cut == 5, "Major CN >= 5", "")
        )
    return(cnv)
}
cnv_plot_hap1 <- set_absolute_pos_and_label(cnv1)
cnv_plot_hap2 <- set_absolute_pos_and_label(cnv2)

plot_cn <- ggplot() +
    geom_segment(data = cnv_plot_hap1,
        aes(
            x = Start,
            xend = End,
            y = cn_cut,
    ...
    geom_segment(data = cnv_plot_hap2,
        aes(
            x = Start,
            xend = End,
            y = cn_cut,
    ...
    labs(
        ...
        title = paste0(
            ...
            ". Each color represents a different haplotype."
        ),
        subtitle = "For visualization purpose, if the major copy number is more than 5, the plot is capped at 5."
    ) +
...
### Copy Number Profile Table

#### Row

```{r, echo=FALSE, message=FALSE, warning=FALSE}
#| title: CNV Table — Haplotype 1 (cnv1)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#| title: CNV Table — Haplotype 2 (cnv2)
# Display everything except those added by the above section
datatable(
    cnv2,
    extensions = "Buttons",
